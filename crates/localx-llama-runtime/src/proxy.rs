...
/// Bind `listen_host` and `listen_port`, then run the no-think proxy until the
/// process exits, forwarding to the upstream named in `config`.
///
/// Use `127.0.0.1` for loopback-only serving or `0.0.0.0` for a LAN gateway;
/// pair a public bind with `api_key`.
///
/// # Errors
...
mod tests {
    ...
    #[tokio::test]
    async fn serve_proxy_on_binds_and_answers_health_over_a_real_socket() {
        let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let port = listener.local_addr().unwrap().port();
        drop(listener);

        let server = tokio::spawn(serve_proxy_on(
            "127.0.0.1",
            port,
            ProxyConfig {
                target_host: "127.0.0.1".into(),
    ...
    #[tokio::test]
    async fn serve_proxy_on_reports_a_bind_conflict() {
        let holder = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let port = holder.local_addr().unwrap().port();
        let err = serve_proxy_on(
            "127.0.0.1",
            port,
            ProxyConfig {
                target_host: "127.0.0.1".into(),
