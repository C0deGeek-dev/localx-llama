...
//! decision); the action they execute comes from [`plan_proxy_lifecycle`].
...
use crate::health::{classify_proxy_target, plan_proxy_action, ProxyAction};
use crate::net::is_port_listening;

/// Plan the proxy lifecycle from observations made by the app's I/O layer.
///
/// `proxy_verified` means the listener answered with the expected health shape
/// and its process identity was recognizable as the LocalX no-think proxy. A
/// listener that fails that check is never killed on inference alone.
#[must_use]
pub fn plan_proxy_lifecycle(
    listener_present: bool,
    proxy_verified: bool,
    upstream_up: bool,
    reported_target: Option<(&str, u16)>,
    wanted_target: (&str, u16),
) -> ProxyAction {
    if listener_present && !proxy_verified {
        return ProxyAction::RefuseForeignListener;
    }
    let target = classify_proxy_target(proxy_verified, reported_target, wanted_target);
    plan_proxy_action(proxy_verified, upstream_up, target)
}
...
mod tests {
    ...
    #[test]
    fn proxy_lifecycle_executes_the_shared_planner_matrix() {
        let wanted = ("127.0.0.1", 8080);
        assert_eq!(
            plan_proxy_lifecycle(false, false, false, None, wanted),
            ProxyAction::Start
        );
        assert_eq!(
            plan_proxy_lifecycle(true, true, true, Some(wanted), wanted),
            ProxyAction::Reuse
        );
        assert_eq!(
            plan_proxy_lifecycle(true, true, true, Some(("127.0.0.1", 9090)), wanted),
            ProxyAction::Repoint
        );
        assert_eq!(
            plan_proxy_lifecycle(true, true, false, Some(wanted), wanted),
            ProxyAction::ReapThenStart
        );
    }

    #[test]
    fn a_foreign_listener_is_refused_not_reaped() {
        assert_eq!(
            plan_proxy_lifecycle(true, false, true, None, ("127.0.0.1", 8080)),
