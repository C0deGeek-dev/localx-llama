...
/// The measurement methodology version accepted by current writers and readers.
///
/// This is deliberately independent of [`TUNER_SCHEMA`]: old store documents
/// stay readable, while entries measured under a superseded methodology are
/// ineligible for launch replay.
pub const CURRENT_TUNER_VERSION: i64 = 5;
...
impl TunerEntry {
    /// Whether this entry was produced by the measurement methodology this
    /// build understands.
    #[must_use]
    pub const fn measurement_supported(&self) -> bool {
        self.tuner_version == CURRENT_TUNER_VERSION
    }
}
...
mod tests {
    ...
    fn deserializes_real_autobest_fixture() {
        ...
        assert_eq!(e.tuner_version, CURRENT_TUNER_VERSION);
        assert!(e.measurement_supported());
    }
    ...
    #[test]
    fn measurement_version_is_independent_of_store_schema() {
        let current: TunerBestConfig = serde_json::from_str(FIXTURE).unwrap();
        assert!(current.schema_supported());
        assert!(current.entries[0].measurement_supported());

        for unsupported in [CURRENT_TUNER_VERSION - 1, CURRENT_TUNER_VERSION + 1] {
            let mut store = current.clone();
            store.entries[0].tuner_version = unsupported;
            assert!(store.schema_supported(), "the document remains readable");
            assert!(!store.entries[0].measurement_supported());
        }
    }

    #[test]
    fn mixed_store_eligibility_is_per_entry() {
        let mut store: TunerBestConfig = serde_json::from_str(FIXTURE).unwrap();
        let mut older = store.entries[0].clone();
        older.quant = "legacy".to_string();
        older.tuner_version = CURRENT_TUNER_VERSION - 1;
        let mut newer = store.entries[0].clone();
        newer.quant = "future".to_string();
        newer.tuner_version = CURRENT_TUNER_VERSION + 1;
        store.entries.extend([older, newer]);
        ...
        let eligible: Vec<_> = store
            ...
            .collect();
        assert_eq!(eligible, ["iq2m"]);
    }

    #[test]
    fn unknown_fields_are_tolerated_without_changing_known_data() {
        let mut value: serde_json::Value = serde_json::from_str(FIXTURE).unwrap();
        value["future_store_field"] = serde_json::json!({"kept_by_raw_readers": true});
        value["entries"][0]["future_entry_field"] = serde_json::json!([1, 2, 3]);

        let store: TunerBestConfig = serde_json::from_value(value).unwrap();
        assert!(store.schema_supported());
        assert!(store.entries[0].measurement_supported());
        assert_eq!(store.entries[0].quant, "iq2m");
    }
}
